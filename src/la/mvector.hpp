...
struct const_iterwrapper
{
  const_iterwrapper(typename container::const_iterator&& iter)
      : __iter(iter)
  {
  }

  const typename container::value_type::second_type& operator*() { return __iter->second; }
  ...
  bool operator<(const const_iterwrapper& other) { return this->__iter < other.__iter; }

  bool operator!=(const const_iterwrapper& other) { return this->__iter != other.__iter; }

  typename container::const_iterator __iter;
};

template <typename container>
struct iterwrapper
{
  iterwrapper(typename container::iterator iter)
      : __iter(iter)
  {
  }

  typename container::value_type::second_type& operator*() { return __iter->second; }
  ...
  bool operator<(iterwrapper& other) { return this->__iter < other.__iter; }
...
struct _value_proxy
{
  _value_proxy(M& m) : m_(m) {}

  auto begin()
  {
    return iterwrapper<M>(m_.begin());
  }

  auto end()
  {
    return iterwrapper<M>(m_.end());
  }

  auto begin() const
  {
    return const_iterwrapper<M>(m_.begin());
  }

  auto end() const
  {
    return const_iterwrapper<M>(m_.end());
  }
...
template <class M>
auto value_proxy(M& m)
{
  return _value_proxy<typename M::container_t>(m.data());
}
...
template<class T>
auto make_mmatrix(std::shared_ptr<MatrixBaseZ> matrix_base)
{
  using memspace = T;
  typedef KokkosDVector<Kokkos::complex<double>**,
                        SlabLayoutV,
                        Kokkos::LayoutStride,
                        memspace,
